...
func _ready() -> void: # Changes colors of scene at the start to match ColorManager's current color
	...
	for label in labels:
		...
		var outline_color_idx = ColorManager._color_in_array(orgColors, label_settings.outline_color)
		
		if font_color_idx != -1:
			label_settings.font_color = Color(ColorManager.colors[ColorManager.currentColor][font_color_idx + 1])
		
		if outline_color_idx != -1:
			label_settings.outline_color = Color(ColorManager.colors[ColorManager.currentColor][outline_color_idx + 1])
...
func _on_colors_changed(oldColors):
	...
	for label in labels: # Changes label settings colors
		...
		var outline_color_idx = ColorManager._color_in_array(oldColors, label_settings.outline_color)
