...
func slide_in():
	...
	tween.tween_property(word_container, "offset_transform_position", Vector2(0, 0), 0.75).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_ELASTIC)
...
func slide_off():
	...
	MusicManager.slide_sound()
	tween.tween_property(word_container, "offset_transform_position", Vector2(-400, 0), 0.75).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUINT)
...
func not_a_word():
	...
	tween.tween_property(current_word_container, "offset_transform_position", Vector2.ZERO, 0.5).set_trans(Tween.TRANS_ELASTIC).set_ease(Tween.EASE_IN_OUT)
