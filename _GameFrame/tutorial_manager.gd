...
func get_ready(start_signal: Signal):
	...
	var target_letter: String = GameManager.solution[GameManager.solution_long.find(
			GameManager.current_word
		)]
	...
	var word_letter_label: Label = get_parent() \
			.get_node("Control/VBoxContainer/CurrentWordHBox") \
			.get_children()[letter_idx]
